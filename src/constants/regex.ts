...
const registerRegex = new RegExp(
  '(' +
    [...Array.from(EightBitRegisters), ...Array.from(SixteenBitRegisters), ...Array.from(ThirtyTwoBitRegisters)].join(
      '|',
    ) +
    ')',
);
...
const regRegRegex = new RegExp(
  registerRegex.source + '\\+' + registerRegex.source + optionalMultByConstantRegex.source,
);
export const regRegOnlyRegex = new RegExp(
  '^(' + registerRegex.source + '\\+' + registerRegex.source + optionalMultByConstantRegex.source + ')$',
);
