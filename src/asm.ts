        ...
        instructions.forEach((ins) => {
            ...
            if (this.jumpMap.has(operation!)) {
                ...
                if (asmFn) {
                    if (ins.label) {
                        if (labels.get(ins.label)) {
                            ...
                            if (unresolvedLabel) {
                                ...
                                const jumPos = this.getPostion(unresolvedLabelPos) + ((unresolvedLabel.jumpSize / 2) + unresolvedLabel.length);
                    ...
                    const machineCode = asmFn.generateMachineCode(ins.operand1?.toLowerCase(), ins.operand2?.toLowerCase(), ins.ptrType);
                    code += machineCode;
                } else {
