...
import {InstructionStructure, PtrType} from '../constants/AsmFunctions';
import {EightBitRegisters, REGISTER_CODE, SixteenBitRegisters, ThirtyTwoBitRegisters} from '../constants/registers';
import {length} from './getTypes';
import {convertToTwosComp} from './twosComplement';
...
import {OpCode, OperandType, types} from '../constants/interfaces';
...
export const generateCode = (
    ...
    if (op2) {
        ...
        if (operand2.isRegisterOnly && operand1?.isMemory && ptrType === 'dword') {
            // define the pointer length when default pointer length is used
    ...
    if (opCode?.length) {
        ...
        if (operand1?.isRegisterOnly && operand2?.isRegisterOnly) {
            ...
            } else if (operand1?.isMemory) {
                // op1 is memory
                if (operand2?.isRegisterOnly) {
                    // op2 is register
