    ...
    if (op.includes('[')) {
        ...
        switch (matchRegex(withoutBrackets)) {
            ...
            case 'reg+reg+disp':
                ...
                const tempArr = [];
                if (t?.includes('mr16')) {
                    //change sib and length for mr16
...
export const length = (s: string, ignore16?: boolean) => {
    if (s.length <= 2) {
        return '8';
    }
    if (s.length <= 4) {
        return !ignore16 ? '16' : '32';
    }
    return '32';
};
